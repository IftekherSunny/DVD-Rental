...
@section('content')
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h4><i class="fa fa-tag"></i> Order Create</h4>
		</div>
		<div class="panel-body">
			<div class="row panel-body-form">
				...
				<form action=" {{ URL::route('order-create-post')}} " method="POST" class="form-horizontal" role="form">						
							...
							<select id="member_id" name="member_id"> 	
							 <option value="" selected>Select your option</option>
								@foreach(Member::all() as $member)
									@if(($member->userActive($member->id)))
										<option {{ e(Input::old('member_id') == $member->id) ? 'selected' : ' '}} value="{{$member->id}}">MEM-{{ $member->id}}</option>
									@endif
								@endforeach
							</select> 
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" {{ e(Input::old('from')) ? ' value="'. Input::old('from') .'"' : ' '}} class="form-control" name="from" placeholder="01-01-2014" data-beatpicker="true" data-beatpicker-format="['DD','MM','YYYY']" data-beatpicker-module="clear">
						</div>
						@if($errors->has('from'))
						<div class="col-sm-4 form-error-msg">
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" {{ e(Input::old('to')) ? ' value="'. Input::old('to') .'"' : ' '}} class="form-control" name="to" placeholder="01-01-2014" data-beatpicker="true" data-beatpicker-format="['DD','MM','YYYY']" data-beatpicker-module="clear">
						</div>
						@if($errors->has('to'))
						<div class="col-sm-4 form-error-msg">
