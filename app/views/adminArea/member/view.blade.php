...
@section('content')
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h4><i class="fa fa-list"></i> All Member List</h4> 
		</div>
			<!-- /.panel-heading -->
			<div class="panel-body">
				<table class="table table-striped table-bordered table-hover" id="dataTables-example">
					...
					@foreach ($members as $member)
						@if($member->user())
							<?php $statusActive = $member->userActive($member->id) ?>
							<tr class="capitalize">
								<td><input type="checkbox" name="checked[]" id="checked" value="{{ $member->id }}"/></td>
								<td>MEM-{{ $member->id }}</td>
								...
								<td><span class="nocapitalize">{{ $member->email }}</span></td>
